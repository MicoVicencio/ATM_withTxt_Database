from atm import ATM  # Importing the ATM class from the atm module
    ...
    def login_account(self):
        while self.login_attempts < self.max_login_attempts:
            if self.userName and self.password == "":
                print("Create account first with numbers and letters!")
                self.welcome_Intro()
        ...
        while True:   
            ...
            if option == "1":
                if self.userName is None:
                    ...
                    new_accounts = input("Do you want to create a new account? (y/n)")
                    if new_accounts == "y":
                        self.new_account()
                        self.userName = input("Enter Username:")
                        self.password = input("Enter Password:")
                ...
                print("Exiting...") 
                self.save_user_data()
                loading_atm(6)  # Display loading animation for ATM operations for 4 seconds
                from main import choose_person
                choose_person()
            else:
